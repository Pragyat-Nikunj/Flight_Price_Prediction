...
class DataIngestion:
    ...
    def initiate_data_ingestion(self):
        ...
        try:
            r_df = pd.read_excel('notebook\data\Data_Train.xlsx')
            df = pd.read_excel('notebook\data\cleaned_flight_data.xlsx')
            logging.info('Read the dataset as dataframe')
            
            os.makedirs(os.path.dirname(self.ingestion_config.train_data_path), exist_ok = True)
            
            r_df.to_csv(self.ingestion_config.raw_data_path, index = True, header = True)
